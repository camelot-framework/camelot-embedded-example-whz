...
public class PluginsContextIntoBeansInjector implements BeanPostProcessor {
    ...
    public Object postProcessBeforeInitialization(Object bean, String beanName) throws BeansException {
        ...
        try {
            injectAnnotatedField(bean.getClass(), bean, Input.class, new AnnotatedFieldListener<EventProducer, Input>() {
                @Override
                public EventProducer found(Field field, Input annotation) throws Exception {
                    Class pluginClass = (Class) getAnnotationValue(annotation, "value");
                    if (!pluginClass.getName().equals(Input.class.getName())) {
                        return interop.forPlugin(pluginClass).input();
                    }
                    return interop.forPlugin((String) getAnnotationValue(annotation, "id")).input();
                }
            });
        } catch (Exception e) {
            logger.error("Could not inject fields annotated with @Plugin into bean " + bean, e);
